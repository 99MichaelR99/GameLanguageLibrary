...
class PostPage extends Component {
  ...
  applyFilters = (posts) => {
    ...
    return posts.filter((post) => {
      const platformMatch =
        platforms.length === 0 || platforms.includes(post.platform);
      const voiceLanguageMatch =
        voiceLanguages.length === 0 ||
        post.voiceLanguages.some((lang) => voiceLanguages.includes(lang));
      const subtitlesLanguageMatch =
        subtitlesLanguages.length === 0 ||
        post.subtitlesLanguages.some((lang) =>
  ...
  getPageData = () => {
    ...
    else filteredPosts = this.applyFilters(allPosts);
