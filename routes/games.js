...
router.post('/', [auth, admin], async (req, res) => {
    ...
    // Check if the game already exists by name
    let game = await Game.findOne({ name: req.body.name });
    if (game) return res.status(400).send('The game already exists with that name.');
    ...
    const uniqueVersions = req.body.versions.filter(version => 
        !uniqueCodes.has(version.code) && uniqueCodes.add(version.code));
    if (uniqueCodes.size !== req.body.versions.length) res.send('Duplicated Version Was Removed');
...
router.post('/:id', [auth], async (req, res) => {
    ...
    const existingVersion = game.versions.find(version => version.code === req.body.code);
    if (existingVersion) return res.status(400).send('The version already exists in the game.');
...
router.put('/:id/versions/:versionId', [auth, admin], async (req, res) => {
    ...
    if (!updatedVersion) return res.status(404).send('The version with the given ID was not found.');
...
// Route to delete a specific version of a game
router.delete('/:id/versions/:versionId', [auth, admin], async (req, res) => {
    // Find the game by ID
    const game = await Game.findById(req.params.id);
    if (!game) return res.status(404).send('The game with the given ID was not found.');

    // Find the index of the version to delete
    const versionIndex = game.versions.findIndex(version => version.id === req.params.versionId);
    if (versionIndex === -1) return res.status(404).send('The version with the given ID was not found.');
